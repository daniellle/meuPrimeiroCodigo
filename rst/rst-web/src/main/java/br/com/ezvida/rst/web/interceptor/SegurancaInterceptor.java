...
abstract class SegurancaInterceptor implements Serializable {
    ...
    Payload validarAutenticacao(InvocationContext context) throws OAuthProblemException, OAuthSystemException, UnsupportedEncodingException {
        ...
        Optional<Object> optional = Arrays.stream(context.getParameters())
                                          .filter(parametro -> parametro instanceof HttpServletRequest)
                                          .findAny();
        ...
        if (payload != null && context.getTarget() instanceof ValidacaoSeguranca && validarCredencial(payload,
            request)) {
