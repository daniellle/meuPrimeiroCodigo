...
import { Component, ViewEncapsulation, ComponentFactoryResolver, ViewChild, Input, OnInit} from '@angular/core';
import {
    OperationalTemplate,
    Composition,
    Locatable,
    ...
    <div class="timeline-item row" >
                                    ...
                                    <div (click)='expandir()' class='col-12 col-lg-11 col-xl-11'>
                                        ...
                                        [style.background-color]='estilo?.cor' [class.expanded] ='expandido'>
                                                    <div class="col" >
                                                        <span>
                                                        <label class='data-encontro'> {{ data | date:'longDate' }} </label>
                                                                <label class='separator'> &#8212; </label>
                                                                <label class='encounter tipo-encontro'> {{ titulo }} </label>
                                                        </span>
                                                    </div>
                                        </div>
...
export class TimelineItemComponent implements OnInit {
    ...
    expandir() {
        ...
        if (expandir) {
            if (!this.dadosCarregados && !this.templateSubscription) {
                this.carregarDados();
            } else {
    ...
    carregarDados() {
        this.templateSubscription = this.service.buscarEncontro(this.elementoHistorico._id).subscribe((data) => {
    ...
    private montarCabecalho(elementoHistorico: { created: Date; composition: any }) {
        ...
        this.titulo = elementoHistorico.composition ? elementoHistorico.composition.name.value : undefined;
    ...
    private processarAtributo(entidade: Element, informacoes: InformacoesTemplate, root: InformacoesTemplate) {
        ...
        if (this.filtrarInformacoes) {
            ...
            if (
                root.arquetipoReferencia === 'openEHR-EHR-ADMIN_ENTRY.ovl-ficha_clinica_ocupacional-historia_ocupacional_atual-001.v1.0.0'
            ) {
                // Função
            ...
            // Exame Físico
            } else if (root.arquetipoReferencia === 'openEHR-EHR-OBSERVATION.ovl-ficha_clinica_ocupacional-exame_fisico_sesi-001.v1.0.0') {
                return;
            // Exame de Auxílio
            } else if  (
                root.arquetipoReferencia === 'openEHR-EHR-EVALUATION.ovl-ficha_clinica_ocupacional-exame_auxilio_diagnostico-001.v1.0.0'
            ) {
                return;
            // Hipóteses diagnosticas
            } else if (
                root.arquetipoReferencia === 'openEHR-EHR-EVALUATION.ovl-ficha_clinica_ocupacional-problem_diagnosis-SESI_BR-002.v1.0.0'
            ) {
                // Conduta
        ...
        if (entidade.value instanceof DvCodedText) {
            ...
            if (entidade.value.upper instanceof DvQuantity && entidade.value.lower instanceof DvQuantity) {
                ...
                if (!isNullOrUndefined(entidade.value.lower.value) || !isNullOrUndefined(entidade.value.upper.value)) {
                    informacoes.pathsComValor.push(entidade);
                }
            }
    ...
    private processEntryObservation(observation: Observation, informacoes: InformacoesTemplate, root: InformacoesTemplate) {
        const info = new InformacoesTemplate();
    ...
    private processEntryInstruction(instruction: Instruction, informacoes: InformacoesTemplate, root: InformacoesTemplate) {
        const info = new InformacoesTemplate();
