...
public class FiltroUnidadeSesi extends FiltroUsuario {
    ...
    public Filtro aplica(UsuarioFilter usuarioFilter, DadosFilter dadosFilter, Usuario usuario) {
        ...
        if(dadosFilter.isGestorDr()) {
            this.filtro.adicionaRestricao("id_und_atd_trab_fk in (select id_und_atd_trabalhador from und_atd_trabalhador where id_departamento_regional_fk in (:idsDrs))",
                    "idsDrs", dadosFilter.getIdsDepartamentoRegional());
        }

        if(dadosFilter.isGetorUnidadeSESI()) {
            this.filtro.adicionaRestricao("id_und_atd_trab_fk in (:idsUats)", "idsUats", dadosFilter.getIdsUnidadeSESI());
        }
